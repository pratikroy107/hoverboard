...
const config = {
  ...
  templateData: [`${getConfigPath()}`, "data/settings", "data/resources"],
  tempDirectory: ".temp"
};
...
const buildPolymerJson = {
  ...
  fragments: polymerJson.fragments.reduce(
    (res, el) => [...res, prependPath(config.tempDirectory, el)],
    []
  ),
  sources: polymerJson.sources.reduce(
    (res, el) => [...res, prependPath(config.tempDirectory, el)],
    []
  ),
...
function build() {
  return new Promise(resolve => {
    ...
    console.log(
      `Deleting ${config.build.rootDirectory}` +
    ...
    del([config.build.rootDirectory, config.tempDirectory])
      .then(() => {
        ...
        const compileStream = template
          .compile(config, polymerJson)
          .on("end", () => {
      ...
      .then(() => {
        ...
        const sourcesStream = polymerProject
          .sources()
        ...
        const dependenciesStream = polymerProject
          .dependencies()
        ...
        let buildStream = mergeStream(sourcesStream, dependenciesStream).once(
          "data",
          () => {
      ...
      .then(() => {
        ...
        const normalizeStream = normalize(config).on("end", () => {
      ...
      .then(() => {
        return gulp
          .src(prependPath(config.build.rootDirectory, "service-worker.js"))
          .pipe(uglify())
...
function lint() {
  return gulp
    .src([`scripts/**/*.js`, `src/**/*.{html,js}`, "index.html"])
    .pipe(eslint())
...
function compileTemplate() {
  return del([config.tempDirectory]).then(() => {
...
function copyStatic() {
  return gulp
    .src(["data/**/*.{markdown,md}", "images/**/*.{png,gif,jpg,svg}"], {
      base: "."
    })
...
function getConfigPath() {
  const path = process.env.BUILD_ENV
    ? `config/${process.env.BUILD_ENV}`
    : "config/development";

  if (!fs.existsSync(`${path}.json`)) {
    console.error(
      `ERROR: Config path '${path}' does not exists. Please, add/use production|development.json files.`
    );
    return null;
  }
